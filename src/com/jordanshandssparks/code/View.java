...
public class View {
    ...
    for (int i = 0; i < numberOfRows; i++) {
      for (int j = 0; j < numberOfColumns; j++) {
        if (field[i][j] instanceof Fox) {
          ...
          if (displayLookingAround && !Field.isRabbitsTurn) {
  ...
  void showLooks(int oldRow, int oldColumn, int newRow, int newColumn) {
    ...
    Vector looks = Field.looks;

    // for each "look," find starting position and direction
    for (int i = 0; i < looks.size(); i++) {
      direction = ((Integer) looks.elementAt(i)).intValue();
      if (direction == Field.STAY) continue;
      int lookRow = oldRow;
      int lookColumn = oldColumn;
      rowDelta = Field.rowChange(direction);
      columnDelta = Field.columnChange(direction);
      lookRow += rowDelta;
